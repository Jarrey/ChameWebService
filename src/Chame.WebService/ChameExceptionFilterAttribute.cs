...
using Unity.WebApi;
using Chame.WebService.Properties;
...
namespace Chame.WebService
{
    ...
    public class ChameExceptionFilterAttribute : ExceptionFilterAttribute
    {
        ...
        {
            // Construct with the logger from Ninject.
            var resolver = GlobalConfiguration.Configuration.DependencyResolver as UnityDependencyResolver;
            if (resolver != null)
            {
                this.logger = resolver.GetService(typeof(ILog)) as ILog;
            }
        }
